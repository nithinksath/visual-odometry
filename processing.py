...
def normalize_points(K_matrices, src_pts, dst_pts):
    ...
    norm_pts1 = np.dot(np.linalg.inv(K_matrices), src_pts)
    norm_pts2 = np.dot(np.linalg.inv(K_matrices), dst_pts)
...
def compute_cam_pose(K_matrices, matched_pts_2D, matched_pts_3D, poses):
    ...
    K1 = K_matrices
    retval,rvec, tvec,inliers = cv2.solvePnPRansac(matched_pts_3D, matched_pts_2D, K1, None)
